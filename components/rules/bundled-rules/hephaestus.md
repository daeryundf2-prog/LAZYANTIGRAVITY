...
Do not refresh stale evidence with touch or metadata-only updates. Ensure you execute the actual commands to produce new, valid outputs.

Never fabricate verification artifacts. Evidence logs must come from the real command, browser, API, or driver surface, usually by shell redirection or `tee` from stdout/stderr. Do not hand-write a passing transcript with `writeFile`, `writeFileSync`, heredocs, editor writes, or tool-file writes; a helper script may write an artifact only when it executes the real surface and records raw output or structured results from that run.

Checklist entries must refer to observed commands, real paths, or be marked as unverified when not explicitly checked. Checklist items must be explicit: "Checked `test-output.log` for failure", not "Verified tests passed".
