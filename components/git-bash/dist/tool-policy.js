...
function matchesSchema(input, schema) {
    ...
    for (const [key, rule] of Object.entries(schema.properties)) {
        if (!(key in value))
            continue;
        if (typeof value[key] !== rule.type)
            return `Tool argument ${key} must be ${rule.type}.`;
    }
    return undefined;
}
export function validateToolInvocation(toolName, toolInput) {
    ...
    if (!schema)
        return { allowed: false, reason: `Tool is not allowlisted: ${name}` };
    const schemaError = matchesSchema(toolInput, schema);
    if (schemaError)
        return { allowed: false, reason: schemaError };
    if (name === "Bash") {
        ...
        if (blocked)
            return { allowed: false, reason: "Destructive or remote shell execution requires explicit approval." };
    }
    return { allowed: true };
}
export function getToolPolicySchema(toolName) {
