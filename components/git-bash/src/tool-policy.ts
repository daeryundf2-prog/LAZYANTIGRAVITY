...
export function validateToolInvocation(toolName: string, toolInput: unknown): ToolPolicyResult {
	if (!toolName.trim()) return { allowed: false, reason: "Tool name is required." };
	if (toolName !== "Bash") return { allowed: true };
	if (!toolInput || typeof toolInput !== "object" || Array.isArray(toolInput)) return { allowed: false, reason: "Bash tool input must be an object." };
	const command = (toolInput as Record<string, unknown>)["command"];
	if (typeof command !== "string" || command.trim() === "") return { allowed: false, reason: "Bash command is required." };
