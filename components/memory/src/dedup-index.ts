...
function readSidecarHashes(
	...
	try {
		const sidecar = JSON.parse(readFileSync(sp, "utf8")) as DedupSidecar;
...
function rebuildHashes(filePath: string): string[] {
...
export function loadDedupIndex(filePath: string): DedupIndex {
	const factsSize = existsSync(filePath) ? statSync(filePath).size : 0;
	const hashes =
		readSidecarHashes(filePath, factsSize) ?? rebuildHashes(filePath);
	...
	if (hashes.length > 0 || existsSync(filePath)) persist();
	return {
		has: (content: string) => set.has(contentHash(content)),
		add: (content: string) => {
