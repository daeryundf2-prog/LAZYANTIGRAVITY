...
export function validateFactReview(value, content, factsPath) {
    try {
        ...
        if (typeof reviewer !== "string" || !reviewer.trim() || typeof reviewedAt !== "string" ||
            ...
            isAbsolute(file) || typeof sha256 !== "string" || !/^[a-f0-9]{64}$/.test(sha256))
            throw new Error();
        ...
        if (receipt["reviewer"] !== reviewer || receipt["reviewedAt"] !== reviewedAt || receipt["decision"] !== decision ||
            receipt["contentSha256"] !== createHash("sha256").update(content.trim()).digest("hex"))
            throw new Error();
...
export function factMetadata(value, content, factsPath, strict = false) {
    const input = record(value);
    const metadata = {};
    if (typeof input["source"] === "string" && input["source"].trim())
        metadata.source = input["source"].trim();
