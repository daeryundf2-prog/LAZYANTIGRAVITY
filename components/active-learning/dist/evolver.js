...
export function getMemoryPath(cwd = process.cwd()) {
    ...
    if (!existsSync(lazyPath))
        mkdirSync(lazyPath, { recursive: true, mode: 0o700 });
    return join(lazyPath, "facts.jsonl");
}
function resolveEvidenceInput(evidenceInput, cwd) {
...
function verifyDiskFilesAndHashes(raw, cwd) {
    ...
    if (!Array.isArray(readRanges) || readRanges.length === 0)
        return "Evidence must contain readRanges";
    if (!Array.isArray(fileChecksums) || fileChecksums.length === 0)
        return "Evidence must contain fileChecksums";
    if (!Array.isArray(raw["commandAudits"]) || raw["commandAudits"].length === 0)
        return "Evidence must contain commandAudits";
    if (!Array.isArray(raw["commandsRun"]) || raw["commandsRun"].length === 0)
        return "Evidence must contain commandsRun";
    if (typeof raw["workspaceFingerprint"] !== "string" || raw["workspaceFingerprint"].trim() === "")
        return "Evidence must contain workspaceFingerprint";
    if (typeof raw["branch"] !== "string" || raw["branch"].trim() === "")
        return "Evidence must contain branch";
    if (typeof raw["workspaceBinding"] !== "string" || raw["workspaceBinding"].trim() === "")
        return "Evidence must contain workspaceBinding";
    let currentBranch = "";
    try {
        currentBranch = execFileSync("git", ["branch", "--show-current"], { cwd, encoding: "utf8" }).trim();
    }
    catch {
        return "Unable to determine current Git branch";
    }
    if (!currentBranch || currentBranch !== raw["branch"])
        return `Evidence branch does not match current branch (${currentBranch || "detached"})`;
    const expectedBinding = createHash("sha256").update(`${currentBranch}:${raw["workspaceFingerprint"]}`, "utf8").digest("hex");
    if (expectedBinding !== raw["workspaceBinding"])
        return "Evidence workspaceBinding does not match current branch and workspace fingerprint";
    ...
    for (const audit of raw["commandAudits"]) {
        ...
        const record = audit;
        if (typeof record["command"] !== "string" || record["exitCode"] !== 0)
            return "Every command audit must have exitCode 0";
    }
    return null;
}
function parseAndValidateEvidence(evidenceInput, cwd) {
    try {
        ...
        if (raw["status"] !== "verified")
            return { valid: false, summary: "", error: `Evidence status must be 'verified' (received '${String(raw["status"])}')` };
        const gaps = ["unknowns", "inferences", "unreadRanges"].filter((key) => Array.isArray(raw[key]) && raw[key].length > 0);
        if (gaps.length > 0)
            return { valid: false, summary: "", error: `Verified evidence contains gaps: ${gaps.join(", ")}` };
...
function sanitizeCandidate(value) {
    ...
    if (normalized.length < 8 || normalized.length > 500)
        return null;
    if (/^(ignore|disregard|forget)\s+(all|any|previous|prior)/i.test(normalized))
        return null;
    return normalized;
}
export function evolveRules(cwd = process.cwd(), options = {}) {
    ...
    if (!shouldPromote)
        return { analyzedEvents: events.length, identifiedPatterns: gotchas.length, promotedGotchas: [] };
    const validation = parseAndValidateEvidence(options.evidenceJson, cwd);
    if (!validation.valid || validation.raw === undefined)
    ...
    for (const gotcha of gotchas) {
        ...
        if (gotcha.confidence < 0.7 || safeRule === null || existingContent.includes(safeRule))
            continue;
        const factRecord = {
            id: gotcha.id,
            timestamp: Date.now(),
