...
export function extractEntities(filePath, content) {
...
function extractTypeScript(file, content) {
    const symbols = [];
    const imports = [];
    const calls = [];
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (fn) {
            ...
            if (typ) {
                const name = typ[1];
                const isExported = /^[A-Z]/.test(name);
                const kind = typ[2] === "struct" ? "struct" : "interface";
...
function collectCalls(trimmed, file, line, caller, calls) {
