...
const ASYNC_FN_RE =
	/(?:export\s+)?(?:async\s+function\s+\w+|async\s*\(|const\s+\w+\s*=\s*async|async\s+\w+\s*\()/;
const RESULT_FN_RE =
	/(?:function|const)\s+([a-zA-Z0-9_$]+)[^={]*(?:=>\s*|:\s*)(?:Promise<\s*)?Result</;
...
function collectResultReturningFns(lines: string[]): Set<string> {
...
export function checkFileBlocking(filePath: string): BlockingFinding[] {
...
export function checkBlocking(targetDir: string): BlockingFinding[] {
	const findings: BlockingFinding[] = [];
