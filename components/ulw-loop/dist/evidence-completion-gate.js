...
export function assertGateUnlocked(repoRoot) {
    const gateFailedFile = ulwLoopGateFailedPath(repoRoot);
    if (existsSync(gateFailedFile)) {
        throw new UlwLoopError("Quality gate is locked due to previous verification failure. Re-verify evidence to unlock.", "ULW_LOOP_GATE_LOCKED");
    }
}
export async function assertGroundTruthEvidence(repoRoot, qualityGateJson, events, claimedEvidence) {
    ...
    try {
        ...
        writeGateFailedMarker(repoRoot, error instanceof Error ? error.message : String(error), error instanceof UlwLoopError ? error.code : "ULW_LOOP_GATE_FAILED");
        throw error;
    }
}
export function writeGateFailedMarker(repoRoot, error, code = "ULW_LOOP_GATE_FAILED") {
    try {
