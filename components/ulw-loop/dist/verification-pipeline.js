...
export function runConsensusGate(ctx, policy) {
    ...
    const needsConsensus = (ctx.riskLevel === "high" && policy.consensusTriggers.riskLevelHigh) ||
...
export function validateConsensusResultEnvelope(envelope, expectedRunId, expectedConsensusId) {
    ...
    if (env.requiresParentAck !== true)
        throw new Error("Validation rejected: requiresParentAck must be true");
    if (env.mayFinalizeRun === true)
        throw new Error("Validation rejected: consensus subagents cannot finalize run");
    if (env.mayChangeModel === true)
        throw new Error("Validation rejected: consensus subagents cannot switch models");
    if (env.wouldSwitchModel === true)
        throw new Error("Validation rejected: consensus subagents cannot switch models");
...
export function calculateConsensusVerdict(results) {
    if (results.length === 0)
        return { type: "quality_gate.consensus_inconclusive", finalizerAllowed: false, parentActionRequired: true };
    let allApprove = true;
    for (const result of results) {
        if (result.verdict === "reject")
            return { type: "quality_gate.consensus_failed", finalizerAllowed: false };
    ...
    if (allApprove)
        return { type: "quality_gate.consensus_passed", finalizerAllowed: true };
    return { type: "quality_gate.consensus_inconclusive", finalizerAllowed: false, parentActionRequired: true };
}
export function calculateQualityFingerprint(evidence) {
    return createHash("sha256").update(JSON.stringify(evidence)).digest("hex");
}
export function runVerificationPipeline(ctx, policy) {
    if (ctx.evidence) {
        const fingerprint = calculateQualityFingerprint(ctx.evidence);
        const existingResult = ctx.events.find((e) => (e.type === "quality_gate.completed" ||
