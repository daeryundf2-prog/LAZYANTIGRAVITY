 ...
 */
export function verifyWalkthroughGroundTruth(repoRoot, walkthroughRelativePath = "walkthrough.md", events = []) {
    const candidates = [
    ...
    const attestedCommands = new Set();
    ...
    for (const cmd of claimedCommands) {
        const hasAttestation = [...attestedCommands].some((attested) => attested === cmd || attested.includes(cmd) || cmd.includes(attested));
    ...
    if (hasPhantoms) {
        ...
        if (phantomFiles.length > 0)
            parts.push(`Phantom files: ${phantomFiles.join(", ")}`);
        if (phantomCommands.length > 0)
            parts.push(`Unverified commands: ${phantomCommands.join(", ")}`);
        return {
            ok: false,
            walkthroughPath: targetPath,
            claimedFiles,
...
export async function verifyWalkthroughCmd(repoRoot, argv, json, scope) {
    ...
    if (json)
        ...
        if (result.skipped) {
            ...
            process.stdout.write(`Walkthrough ground-truth verified: ${result.claimedFiles.length} file(s), ${result.claimedCommands.length} command(s) attested.\n`);
        }
    }
    else {
