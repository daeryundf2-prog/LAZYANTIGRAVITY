...
function required(argv, flag) {
    ...
    if ((flag === "--run-id" || flag === "--agent-id" || flag === "--consensus-id") &&
...
export async function dispatchConsensusCmd(repoRoot, argv, json) {
    ...
    if (json)
        ...
        process.stdout.write(`Dispatched consensus ${result.consensusId} for run ${runId}.\n`);
    return 0;
}
export async function reportConsensusResultCmd(repoRoot, argv, json) {
    const runId = required(argv, "--run-id");
...
export async function aggregateConsensusCmd(repoRoot, argv, json) {
    ...
    if (json)
        printJson({ ok: true, consensusId, verdict });
    else
        process.stdout.write(`Aggregated consensus ${consensusId}. Verdict: ${verdict}\n`);
    return 0;
}
