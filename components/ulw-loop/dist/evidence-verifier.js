...
const FAILURE_KEYWORDS = ["fail", "error", "exception", "failed", "unhandledrejection", "rejected"];
...
function extractPhysicalEvidencePath(evidenceStr) {
    ...
    const fileUrl = cleanupSeparator?.index === undefined
        ? fileUrlAndTail.trim()
        : fileUrlAndTail.slice(0, cleanupSeparator.index).trim();
    return fileURLToPath(fileUrl);
}
export function physicalEvidenceFreshness(stats, referenceTimeMs) {
...
export function verifyPhysicalEvidenceFile(repoRoot, evidenceStr) {
