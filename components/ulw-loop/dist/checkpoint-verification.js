...
async function readJsonInput(raw, repoRoot) {
    ...
    if (!existsSync(path))
        throw new UlwLoopError("Quality gate JSON is neither valid JSON nor a readable path.", "ulw_loop_json_input_invalid");
    try {
        return JSON.parse(await readFile(path, "utf8"));
    }
    catch (error) {
        throw new UlwLoopError(`Quality gate path does not contain valid JSON${error instanceof Error ? `: ${error.message}` : "."}`, "ulw_loop_json_input_invalid");
    }
}
...
export async function runCheckpointQualityGate(repoRoot, goal, plan, evidence, args, now, scope) {
    ...
    if (existingFailure) {
        ...
        if (lastMechFailed) {
            ...
            goalStatusOverride = "failed";
            failedReasonOverride = existingFailure.reason || "Verification pipeline failed";
        }
        return {
    ...
    const isSecuritySensitive = /\b(security|auth|login|password|encrypt|token|credential|permission)\b/i.test(goal.objective) ||
        /\b(security|auth|login|password|encrypt|token|credential|permission)\b/i.test(evidence);
    const isPublicRelease = /\b(release|publish|deploy|production|public)\b/i.test(goal.objective) ||
        /\b(release|publish|deploy|production|public)\b/i.test(evidence);
    const isDestructive = /\b(delete|remove|destroy|drop|truncate|destructive)\b/i.test(goal.objective) ||
    ...
    if (!reconciliation.ok) {
        ...
        if (!taskScoped)
            throw new UlwLoopError(`${formatCodexGoalReconciliation(reconciliation)}${aggregate && snapshot?.status === "complete" && objective !== undefined
