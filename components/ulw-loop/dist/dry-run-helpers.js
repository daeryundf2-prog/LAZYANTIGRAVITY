...
export function out(json, msg) {
    if (!json) {
        process.stdout.write(`[Dry-Run] ${msg}\n`);
    }
}
export async function saveDryRunCheckpoint(ctx, state, taskId) {
    if (!ctx.writeCheckpoint)
        return;
...
export function cleanupRunDir(writeCheckpoint, writeLedger, runDir) {
    if (!writeCheckpoint && !writeLedger && existsSync(runDir)) {
        rmSync(runDir, { recursive: true, force: true });
    }
}
export function printFinalOutput(state, ctx, scenario) {
    ...
    if (ctx.json) {
        ...
        printJson({
            ...
            ...(state.isQualityScenario && {
                ...
                eventType: state.qualityStatus === "passed"
