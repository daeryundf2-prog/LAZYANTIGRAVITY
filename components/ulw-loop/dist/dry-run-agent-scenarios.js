...
export async function dispatchAgentScenario(scenario, ctx) {
...
async function subagentSelfFinalizes(ctx) {
...
async function staleHeartbeatMissed(ctx) {
    ...
    try {
        ...
        if (!ctx.json) {
            ...
            process.stdout.write(`[Dry-Run] Agent worker-stale assignment state: ${leaseState.agents["worker-stale"]?.state}\n`);
        }
    }
    finally {
        cleanupRunDir(ctx.writeCheckpoint, ctx.writeLedger, runDir);
...
async function pollingLoopPrevented(ctx) {
...
async function parentProgressReconstruct(ctx) {
...
async function subagentWrongRoleEnvelope(ctx) {
