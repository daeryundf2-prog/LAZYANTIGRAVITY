...
function extractFingerprints(event) {
    let errorHash = "";
    let patchHash = "";
...
export function checkStagnation(events, policy) {
    if (events.length < policy.minimumEventsForDetection)
        return { status: "ok" };
    ...
    for (const ev of recentEvents) {
        ...
        if (patchHash)
            patchHashes.push({ hash: patchHash, role: ev.role });
    }
    const buildPayload = (kind, fingerprint, windowSize, threshold, suggestedParentAction) => ({
        runId,
        ...(agentId ? { agentId } : {}),
        ...(role ? { role } : {}),
        kind,
