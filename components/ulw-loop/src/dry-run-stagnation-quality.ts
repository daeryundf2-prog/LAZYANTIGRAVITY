...
export function dispatchStagnationQualityScenario(scenario: string, ctx: DryRunContext, state: DryRunState): boolean {
...
function sameErrorLoop(ctx: DryRunContext, state: DryRunState): void {
...
function oscillatingPatch(ctx: DryRunContext, state: DryRunState): void {
	state.completedRoles = ["planner"];
	state.failedRole = null;
	state.nextRecommendedAction = "Parent handles stagnation: pause, replan, or human intervention";
	if (!ctx.json) {
...
function heartbeatOnlyStall(ctx: DryRunContext, state: DryRunState): void {
	state.completedRoles = ["planner"];
	state.failedRole = null;
	state.nextRecommendedAction = "Parent handles stagnation: pause, replan, or human intervention";
	if (!ctx.json) {
...
function noEvidenceProgress(ctx: DryRunContext, state: DryRunState): void {
	state.completedRoles = ["planner"];
	state.failedRole = null;
	state.nextRecommendedAction = "Parent handles stagnation: pause, replan, or human intervention";
	if (!ctx.json) {
...
function qualityHappyPath(ctx: DryRunContext, state: DryRunState): void {
...
function qualityMechanicalFail(ctx: DryRunContext, state: DryRunState): void {
	state.isQualityScenario = true;
...
function qualitySemanticInsufficientEvidence(ctx: DryRunContext, state: DryRunState): void {
	state.isQualityScenario = true;
	state.qualityStatus = "failed";
...
function qualityConsensusRequired(ctx: DryRunContext, state: DryRunState): void {
	state.isQualityScenario = true;
...
function qualityStagnationUnresolved(ctx: DryRunContext, state: DryRunState): void {
	state.isQualityScenario = true;
	state.qualityStatus = "failed";
	state.qualityStage = "semantic";
	if (!ctx.json) {
...
function hitlScenario(ctx: DryRunContext, state: DryRunState): void {
	state.completedRoles = ["planner"];
	state.failedRole = null;
