...
export function generateReworkSuggestions(lspDiagnostics: string[], rulesViolations: string[]): string {
	...
	if (lspDiagnostics.length >= 3) {
		suggestions.push(`- WARNING: ${lspDiagnostics.length} LSP errors detected. Consider switching to Gemini 3.6 Flash (Medium) for rapid iterative bug fixes before re-attempting the checkpoint.`);
	}
