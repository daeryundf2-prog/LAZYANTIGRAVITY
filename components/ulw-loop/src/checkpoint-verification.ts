...
export async function runCheckpointQualityGate(
	...
	if (existingFailure) {
		...
		const conFailed = events.find((e) =>
			(e.type === "quality_gate.consensus_failed" || e.type === "quality_gate.consensus_inconclusive" || e.type === "quality_gate.consensus_rework_required") &&
	...
	if (isSecuritySensitive || isPublicRelease || isDestructive || filesChanged.length > 5 || lspDiagnostics.length > 0 || rulesViolations.length > 0) riskLevel = "high";
	else if (filesChanged.length > 2) riskLevel = "medium";

	const ctx: VerificationContext = { runId, events, evidence: evidenceEnvelope, goal: goal.objective, wouldSwitchModel: false, isDryRun: true, riskLevel, destructiveChange: isDestructive, publicRelease: isPublicRelease, securitySensitive: isSecuritySensitive, lspDiagnostics, rulesViolations };
