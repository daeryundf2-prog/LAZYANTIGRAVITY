...
const ALL_PERSONAS: ConsensusPersona[] = [
	"advocate",
	"devils_advocate",
	"regression_reviewer",
...
export interface LiveConsensusClient {
	...
	pollMessages(sessionId: string, timeoutMs: number): Promise<{ text: string; structuredOutput?: any }>;
}
...
export function validateConsensusSchema(envelope: any): void {
...
export class OpenCodeLiveConsensusClient implements LiveConsensusClient {
	...
	async sendMessage(sessionId: string, text: string, schema?: any): Promise<void> {
	...
	async pollMessages(sessionId: string, timeoutMs: number): Promise<{ text: string; structuredOutput?: any }> {
...
export class MockLiveConsensusClient implements LiveConsensusClient {
	...
	async sendMessage(_sessionId: string, _text: string, _schema?: any): Promise<void> {
		// Mock save session mapping is handled externally if needed
	}

	async pollMessages(sessionId: string, _timeoutMs: number): Promise<{ text: string; structuredOutput?: any }> {
...
const mockSessionToPersona: Record<string, string> = {};
const mockPersonaVerdict: Record<string, string> = {};

export function setMockPersonaVerdict(persona: ConsensusPersona, verdict: string) {
...
async function waitForResult(
	...
	sessionId: string,
	timeoutMs: number,
): Promise<{ text: string; structuredOutput?: any }> {
...
function getLatestAssistantMessage(messages: any[]): any {
