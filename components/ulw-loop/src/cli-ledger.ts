...
export async function verifyLedgerCmd(
	repoRoot: string,
	...
	if (json) printJson({ ok: result.valid, ...result });
	else if (result.valid) {
		process.stdout.write(
			`Ledger integrity verified for run ${runId}: ${result.eventCount} event(s), hash chain valid.\n`,
		);
	} else {
		process.stdout.write(
			`Ledger integrity FAILED for run ${runId}: index ${result.brokenIndex} (${result.eventCount} events):\n`,
		);
