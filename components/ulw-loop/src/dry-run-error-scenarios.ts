...
function happyPath(ctx: DryRunContext, state: DryRunState): void {
...
async function quotaOpusExhausted(ctx: DryRunContext, state: DryRunState): Promise<boolean> {
	...
	if (!ctx.json) {
		...
		process.stdout.write(
			`[Dry-Run] Fallback Recommendation: Switch to Gemini 3.7 Flash (High) in Antigravity UI and run /ulw resume.\n`,
		);
...
async function contextWindowExceeded(ctx: DryRunContext, state: DryRunState): Promise<boolean> {
...
async function outputTokenLimit(ctx: DryRunContext, state: DryRunState): Promise<boolean> {
	state.completedRoles = ["planner", "researcher"];
	state.failedRole = "worker";
...
async function providerUnavailable(ctx: DryRunContext, state: DryRunState): Promise<boolean> {
	state.completedRoles = ["planner"];
	state.failedRole = "researcher";
