...
export function executionCommand(command: string, args: readonly string[]): string {
	return [command, ...args].map((part) => (/^[\w./:@=-]+$/.test(part) ? part : JSON.stringify(part))).join(" ");
}

function recordPath(repoRoot: string, binding: ExecutionBinding): string {
...
export function verifyExecutionRecords(repoRoot: string, evidence: StrictEvidenceEnvelope): string[] {
	const errors: string[] = [];
	const commands = evidence.commandsRun ?? [];
	const audits = evidence.commandAudits ?? [];
	if (!commands.length && !audits.length && !evidence.executionBinding) return errors;
	const binding = evidence.executionBinding;
	if (!binding) return ["Missing trusted execution binding"];
	const check = (candidate: ExecutionBinding, command?: string): boolean => {
		try {
			const record: ExecutionRecord = JSON.parse(readFileSync(recordPath(repoRoot, candidate), "utf8"));
			return (
				record.workspaceRoot === realpathSync(repoRoot) &&
				...
				(Object.keys(record.binding) as (keyof ExecutionBinding)[]).every(
					(key) => record.binding[key] === candidate[key],
				) &&
				candidate.exitCode === 0 &&
				Number.isFinite(Date.parse(candidate.startedAt)) &&
				Date.parse(candidate.finishedAt) >= Date.parse(candidate.startedAt) &&
				[candidate.stdoutFingerprint, candidate.stderrFingerprint].every(
					(value) => /^[a-f0-9]{64}$/.test(value) && !/^0+$/.test(value),
				) &&
	...
	if (!check(binding)) errors.push("Execution binding does not match a trusted host execution record");
	for (const command of commands) {
		if (!audits.some((audit) => audit.command === command)) errors.push(`Missing command audit for "${command}"`);
	}
	for (const audit of audits) {
		const candidate = audit.executionBinding ?? binding;
		if (
			!check(candidate, audit.command) ||
