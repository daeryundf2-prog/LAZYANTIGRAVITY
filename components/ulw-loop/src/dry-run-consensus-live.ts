...
export async function runConsensusLiveInvocation(ctx: DryRunContext, state: DryRunState): Promise<void> {
	...
	try {
		const { dispatchConsensus, setMockPersonaVerdict, reportConsensusResult, aggregateConsensus } = await import(
			"./consensus-dispatcher.js"
		);
		const { appendRunEvent, readRunEvents } = await import("./control-plane.js");

		if (!ctx.json) process.stdout.write(`[Dry-Run] Case 1: All personas approve...\n`);
		...
		if (!ctx.json) process.stdout.write(`[Dry-Run] Case 2: Devil's Advocate rejects...\n`);
		setMockPersonaVerdict("advocate", "approve");
		...
		if (!ctx.json) process.stdout.write(`[Dry-Run] Case 3: Regression Reviewer requires rework...\n`);
		setMockPersonaVerdict("advocate", "approve");
		setMockPersonaVerdict("devils_advocate", "approve");
		...
		if (!ctx.json) process.stdout.write(`[Dry-Run] Case 4: Persona times out...\n`);
		setMockPersonaVerdict("advocate", "approve");
		setMockPersonaVerdict("devils_advocate", "approve");
		setMockPersonaVerdict("regression_reviewer", "approve");
		...
		if (!ctx.json) process.stdout.write(`[Dry-Run] Case 5: Invalid Envelope submitted...\n`);
		setMockPersonaVerdict("advocate", "approve");
		setMockPersonaVerdict("devils_advocate", "approve");
		setMockPersonaVerdict("regression_reviewer", "approve");
		...
		if (!ctx.json) process.stdout.write(`[Dry-Run] Case 6: Duplicate Same Payload reported...\n`);
		setMockPersonaVerdict("advocate", "approve");
		setMockPersonaVerdict("devils_advocate", "approve");
		setMockPersonaVerdict("regression_reviewer", "approve");
		setMockPersonaVerdict("security_state_reviewer", "approve");
		...
		if (!ctx.json) process.stdout.write(`[Dry-Run] Case 7: Duplicate Conflicting Payload reported...\n`);
