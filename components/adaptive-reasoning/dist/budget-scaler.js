const ACTION_MODIFICATION_PATTERN = /수정|구현|작성|추가|생성|개발|빌드|고쳐|만들어|디버깅|패치|설계|fix|implement|create|add|write|build|update|refactor|generate|debug|patch/i;
const PURE_INQUIRY_PATTERN = /^(어디|위치|설명|찾아|오타|단순|간단|확인|알려|보여|조회|explain|where|what|how|why|which|show|find)/i;
export function computeThinkingBudget(prompt) {
    ...
    if (/security|vulnerability|adversarial|audit|falsification|hypothesis-tree|arch-guard|architecture drift|clean architecture|race condition|exploit|consensus|보안|취약점|인젝션|침투|감사|반증|동시성|레이스|합의/i.test(lower)) {
        return {
            budget: 64000,
            tier: "pro",
            level: "deep",
    ...
    if (/ultrawork|\/ulw|ulw-plan|start-work|refactor|flaky-guard|flaky test|swarm-sync|worktree|mutation|ui-loopback|visual-qa|리팩토링|아키텍처|스트레스|자가 치유|돌연변이|워크트리|오케스트레이션/i.test(lower)) {
        return {
            budget: 32768,
            tier: "flash",
            level: "high",
