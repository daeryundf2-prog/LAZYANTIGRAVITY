...
test("#given source package versions and component without hooks #when hook status messages sync #then source versions are preserved", async () => {
	...
	await writeJson(join(root, "plugin.json"), { version: "0.1.0" });
	...
	assert.equal(aggregateHooks.hooks.PostToolUse[0].hooks[0].statusMessage, "LazyAntigravity(0.1.0): Checking Comments");
	assert.equal(aggregateHooks.hooks.PostToolUse[0].hooks[1].statusMessage, "LazyAntigravity(0.1.0): Checking LSP Diagnostics");
	assert.equal(componentHooks.hooks.PostToolUse[0].hooks[0].statusMessage, "LazyAntigravity(0.1.1): Checking Comments");
	assert.equal(lspHooks.hooks.PostToolUse[0].hooks[0].statusMessage, "LazyAntigravity(0.2.0): Checking LSP Diagnostics");
});
...
test("#given release version override #when hook status messages sync #then aggregate hooks use release version", async () => {
	...
	const aggregateHooks = await readJson(join(root, "hooks", "hooks.json"));
	const componentHooks = await readJson(join(root, "components", "comment-checker", "hooks", "hooks.json"));
	assert.equal(aggregateHooks.hooks.PostToolUse[0].hooks[0].statusMessage, "LazyAntigravity(4.8.0): Checking Comments");
	assert.equal(componentHooks.hooks.PostToolUse[0].hooks[0].statusMessage, "LazyAntigravity(4.8.0): Checking Comments");
});
