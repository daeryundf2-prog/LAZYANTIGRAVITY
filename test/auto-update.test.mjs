...
import { resolveAutoUpdatePlan, resolveAutoUpdateStatus, resolveLazyCodexUpdatePlan, runAutoUpdateCheck } from "../scripts/auto-update.mjs";
import { resolveLogPath, resolveLockPath, resolveStatePath } from "../scripts/auto-update-state.mjs";
...
test("#given stale state without explicit update source #when resolving plan #then status only is scheduled", () => {
	const plan = resolveAutoUpdatePlan({
	...
	assert.equal(plan.shouldRun, false);
	assert.equal(plan.mode, "status");
	assert.equal(plan.reason, "missing-update-source");
	assert.doesNotMatch(JSON.stringify(plan), /lazycodex-ai@latest/);
});

test("#given LazyAntigravity default env #when resolving plan #then no lazycodex installer is scheduled", () => {
	const plan = resolveAutoUpdatePlan({
		env: {
			ANTIGRAVITY_CURRENT_VERSION: "1.0.0",
			ANTIGRAVITY_LATEST_VERSION: "1.0.1",
			ANTIGRAVITY_HOME: "/tmp/antigravity-home",
		},
		now: 90_000_000,
		lastCheckedAt: 0,
	});
...
test("#given explicit Antigravity update source #when resolving plan #then source command is scheduled", () => {
	const plan = resolveAutoUpdatePlan({
		env: {
			ANTIGRAVITY_CURRENT_VERSION: "1.0.0",
			ANTIGRAVITY_LATEST_VERSION: "1.0.1",
			ANTIGRAVITY_AUTO_UPDATE_SOURCE: "lazyantigravity@latest",
		},
		now: 90_000_000,
		lastCheckedAt: 0,
	});
	...
	assert.deepEqual(plan.args, ["--yes", "lazyantigravity@latest", "install", "--no-tui", "--codex-autonomous"]);
	assert.equal(plan.legacyCompatibility, false);
	assert.doesNotMatch(JSON.stringify(plan), /lazycodex-ai@latest/);
});

test("#given explicit legacy compatibility source #when resolving plan #then source is marked legacy", () => {
	const plan = resolveAutoUpdatePlan({
		env: {
			LAZYCODEX_CURRENT_VERSION: "1.0.0",
			LAZYCODEX_LATEST_VERSION: "1.0.1",
			LAZYCODEX_AUTO_UPDATE_SOURCE: "lazycodex-ai@latest",
		},
		now: 90_000_000,
		lastCheckedAt: 0,
	});
	...
	assert.equal(plan.legacyCompatibility, true);
});

test("#given malformed Antigravity args JSON #when resolving plan #then fails closed", () => {
	assert.throws(
		() => resolveAutoUpdatePlan({
			env: {
				ANTIGRAVITY_CURRENT_VERSION: "1.0.0",
				ANTIGRAVITY_LATEST_VERSION: "1.0.1",
				ANTIGRAVITY_AUTO_UPDATE_ARGS_JSON: JSON.stringify([""]),
			},
			now: 90_000_000,
			lastCheckedAt: 0,
		}),
		/ANTIGRAVITY_AUTO_UPDATE_ARGS_JSON must be a non-empty JSON string array/,
	);
});
...
test("#given Antigravity state env #when resolving paths #then Antigravity names take precedence", async () => {
	const root = await mkdtemp(join(tmpdir(), "lazyantigravity-auto-update-state-"));
	try {
		const env = {
			ANTIGRAVITY_PLUGIN_DATA: join(root, "antigravity-data"),
			LAZYCODEX_PLUGIN_DATA: join(root, "legacy-data"),
		};
		const statePath = resolveStatePath(env);

		assert.equal(statePath, join(root, "antigravity-data", "auto-update.json"));
		assert.equal(resolveLogPath(env), join(root, "antigravity-data", "auto-update.log"));
		assert.equal(resolveLockPath(env, statePath), join(root, "antigravity-data", "auto-update.json.lock"));
	} finally {
		await rm(root, { recursive: true, force: true });
	}
});

test("#given status request #when resolving status #then no mutating command is exposed by default", async () => {
	const root = await mkdtemp(join(tmpdir(), "lazyantigravity-auto-update-status-"));
	try {
		const status = resolveAutoUpdateStatus({
			env: {
				ANTIGRAVITY_PLUGIN_DATA: join(root, "data"),
				ANTIGRAVITY_CURRENT_VERSION: "1.0.0",
				ANTIGRAVITY_LATEST_VERSION: "1.0.1",
			},
			now: 90_000_000,
		...
		assert.equal(status.product, "LazyAntigravity");
		assert.equal(status.mode, "status");
		assert.equal(status.dryRun, true);
		assert.equal(status.mutating, false);
		assert.equal(status.commandPlan, null);
		assert.doesNotMatch(JSON.stringify(status), /lazycodex-ai@latest/);
	} finally {
		await rm(root, { recursive: true, force: true });
	}
});

test("#given hook invocation with explicit hung update source #when launched without run flag #then status path does not execute installer", async () => {
	const root = await mkdtemp(join(tmpdir(), "lazyantigravity-auto-update-hook-status-"));
	try {
		const statePath = join(root, "state.json");
		const logPath = join(root, "auto-update.log");
		const sentinelPath = join(root, "installer-ran.txt");
		const result = spawnSync(process.execPath, ["scripts/auto-update.mjs", "status"], {
			...
			env: {
				...
				ANTIGRAVITY_CURRENT_VERSION: "1.0.0",
				ANTIGRAVITY_LATEST_VERSION: "1.0.1",
				ANTIGRAVITY_AUTO_UPDATE_INTERVAL_MS: "0",
				ANTIGRAVITY_AUTO_UPDATE_STATE_PATH: statePath,
				ANTIGRAVITY_AUTO_UPDATE_LOG_PATH: logPath,
				ANTIGRAVITY_AUTO_UPDATE_COMMAND: process.execPath,
				ANTIGRAVITY_AUTO_UPDATE_ARGS_JSON: JSON.stringify([
					"-e",
					`require("node:fs").writeFileSync(${JSON.stringify(sentinelPath)}, "ran"); setTimeout(() => {}, 10_000);`,
				]),
			},
		});
...
test("#given current version #when resolving update plan #then skips installer", () => {
	const plan = resolveLazyCodexUpdatePlan({
		...
		latestVersion: "1.0.1",
		args: ["--yes", "lazyantigravity@latest", "install"],
	});
...
test("#given latest version and explicit Antigravity source #when resolving plan #then schedules installer", () => {
	const plan = resolveLazyCodexUpdatePlan({
	...
	assert.deepEqual(plan.args, ["--yes", "lazyantigravity@latest", "install"]);
	assert.doesNotMatch(JSON.stringify(plan), /lazycodex-ai@latest/);
});

test("#given current version is a prerelease and explicit source #when resolving update plan #then schedules stable installer", () => {
	const plan = resolveLazyCodexUpdatePlan({
...
test("#given current version #when resolving auto update plan #then no command is scheduled", () => {
	const plan = resolveAutoUpdatePlan({
		env: {
			ANTIGRAVITY_CURRENT_VERSION: "1.0.1",
			ANTIGRAVITY_LATEST_VERSION: "1.0.1",
			ANTIGRAVITY_AUTO_UPDATE_SOURCE: "lazyantigravity@latest",
		},
		now: 90_000_000,
...
test("#given active lock #when running check #then skips concurrent update", async () => {
	...
	try {
		...
		assert.match(await readFile(join(root, "codex-home", "config.toml"), "utf8"), /model_context_window = 400_?000/);
	} finally {
		await rm(root, { recursive: true, force: true });
	}
});
