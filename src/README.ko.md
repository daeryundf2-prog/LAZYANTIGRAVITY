...
- [전체 스킬 카탈로그 (34개)](#-전체-스킬-카탈로그-34개)
- [디지털 포렌식 & 데이터 분석 도구](#-디지털-포렌식--데이터-분석-도구)
...
> - **전문화된 스킬 (25개)**: 코딩 루프 실행 중 코드를 수정하거나 검증할 때 백그라운드 훅(Lifecycle Hooks)으로 자동 연동되거나 필요에 따라 엔진에 의해 호출(Call)되는 **도구 및 품질 게이트** 역할을 합니다.
...
### ![Code Quality](https://img.shields.io/badge/Code_Quality-FBBC05?style=flat-square) 코드 품질 (6개)
...
| **cjk-cleaner** | `clean cjk spacing`, `normalize cjk`, `cjk-cleaner`, `CJK spacing slop`, `clean text spacing` | CJK 동적 띄어쓰기 정규화기. 불필요한 공백 제거, 자모 NFC 통합, 다국어 경계 공백 교정 |

### ![Digital Forensics & Data Analysis](https://img.shields.io/badge/Forensics_%26_Analysis-0F172A?style=flat-square) 디지털 포렌식 & 데이터 분석 (4개)

| 스킬 | 트리거 | 설명 |
| :--- | :--- | :--- |
| **exif-extractor** | `exif`, `metadata`, `hashes`, `spoofing`, `signature check` | 파일 메타데이터 및 해시(MD5, SHA-256) 추출, 파일 매직 시그니처 검증을 통한 확장자 변조 감지 |
| **sqlite-analyzer** | `sqlite`, `db`, `query database`, `database schema` | SQLite 데이터베이스 스키마 검사, 레코드 수 조회, 테이블 내 키워드 패턴 검색 |
| **macb-timeline** | `timeline`, `MACB`, `directory timeline`, `file events` | 디렉터리 내 파일들로부터 연대기적 MACB(수정, 접근, 변경, 생성) 타임라인 생성 |
| **duplicate-finder** | `duplicate finder`, `find duplicates`, `check duplicates`, `md5 match` | 디렉터리를 재귀적으로 스캔하여 MD5 해시 기준 중복 파일을 찾아 분류 |
...
lazyantigravity는 보안 연구원, 디지털 포렌식 조사관, 데이터 분석가를 위해 설계된 전용 도구 모음을 제공합니다. 이 스크립트들은 깊은 콘텐츠 검증, 파일 메타데이터 스캔, 데이터베이스 탐색 및 중복 감지를 자동화합니다:

1. **`duplicate-finder` (MD5 중복 파일 탐지기)**:
   - **실행 명령**: `node scripts/find-duplicates.mjs <directory_path>`
   - **최적화**: 고성능 2패스(two-pass) 알고리즘을 사용합니다. 먼저 모든 파일을 크기별로 그룹화한 뒤, 크기가 일치하는 파일들에 대해서만 MD5 해시를 계산하므로 디스크 읽기 연산(I/O)을 극적으로 절감합니다.
   - **출력**: 중복 파일들을 MD5 해시 및 크기별로 명확하게 분류하여, 절대 경로와 함께 표시합니다.

2. **`exif-extractor` (Exif 메타데이터 및 확장자 위변조 탐지기)**:
   - **실행 명령**: `node scripts/extract-metadata.mjs <file_path>`
   - **설명**: 파일의 Exif 메타데이터를 추출하고 MD5 및 SHA-256 해시를 계산하며, 매직 시그니처(JPEG, PNG, ZIP, PDF, SQLite, EXE 등)를 분석하여 실제 포맷과 확장자가 일치하는지 위변조 여부를 체크합니다.

3. **`sqlite-analyzer` (SQLite 스키마 검사 및 키워드 탐색기)**:
   - **실행 명령**: `node scripts/analyze-sqlite.mjs <db_path>`
   - **설명**: SQLite 데이터베이스 스키마 구조를 자동으로 추출하고, 모든 테이블의 레코드 수를 계산하며, 데이터베이스 내 모든 텍스트 필드를 대상으로 딥 키워드 패턴 검색을 수행합니다.

4. **`macb-timeline` (시간대별 파일 이벤트 타임라인 생성기)**:
   - **실행 명령**: `node scripts/generate-timeline.mjs <directory_path>`
   - **설명**: 디렉터리를 재귀적으로 스캔하여 모든 파일의 MACB(수정, 접근, 변경, 생성) 타임스탬프를 추출하고, 시간 순서에 따른 타임라인 보고서를 작성합니다.
...
- **29개 전문 스킬**: ultraresearch 스웜, 시각 QA, TDD 워크플로우, 팀 오케스트레이션 등.
...
## 🛡️ 최근 안정성 및 프라이버시 보강 사항

1. **의존성 내부화 구체화 (Sibling Dependency Materialization)**:
   - 플러그인 빌드 및 캐싱 과정에서 외부의 로컬 형제 패키지 의존성(예: `shared-skills`, `shared-lsp` 등)이 누락되는 배포 결함을 원천 제거했습니다. 캐시 설치 프로그램이 이러한 의존 폴더의 실물을 플러그인 캐시 디렉터리로 복사(materialize)한 후, `package.json` 의존성 주소를 `file:./shared-lsp` 와 같이 내재화하도록 패키징 흐름을 보강했습니다.
2. **개인정보 보호 강화 (PostHog GeoIP 비활성화)**:
   - 텔레메트리 클라이언트 엔진에서 PostHog GeoIP 위치 확인(`disableGeoip: true`)을 명시적으로 차단하여, 가명의 최소 기기 지표 외에는 IP나 지리적 정보를 수집하지 않습니다.
3. **시스템 닥터(Doctor) 검사 엄격화**:
   - 로드된 플러그인 버전과 실제 기대되는 플러그인 캐시 버전이 일치하지 않을 경우 발생할 수 있는 에이전트의 런타임 오동작을 미연에 감지하도록, 닥터 진단 검사의 위험 수준을 `warning`에서 `error`로 격상시켰습니다.
...
사용자 프라이버시 보호를 위해 텔레메트리 전송 시 **PostHog GeoIP 위치 조회가 기본으로 비활성화**(`disableGeoip: true`)되어 기기 위치가 추적되지 않습니다. 세션 시작 시 하루에 한 번, 식별을 위해 해시된 호스트네임(`sha256("omo-codex:" + hostname)`)을 기반으로 한 가명의 기기 레벨 지표만 수집됩니다. **소스 코드, 파일 내용, 또는 비밀키 등 민감 데이터는 절대로 외부로 전송되지 않습니다.**

모든 텔레메트리를 비활성화하려면 아래 환경 변수를 주입하십시오:
```bash
