...
- [Complete Skill Catalog (34 Skills)](#-complete-skill-catalog-34-skills)
- [Digital Forensics & Data Analysis Tools](#-digital-forensics--data-analysis-tools)
...
## 📦 Complete Skill Catalog (34 Skills)
...
> - **Specialized Skills (25 skills)**: Act as the **"Tools/Quality Gates"** called by the engine or triggered automatically by lifecycle hooks (like type checking, comment protection, and pixel-diff QA) during execution.
...
### ![Code Quality](https://img.shields.io/badge/Code_Quality-FBBC05?style=flat-square) Code Quality (6 skills)
...
| **cjk-cleaner** | `clean cjk spacing`, `normalize cjk`, `cjk-cleaner`, `CJK spacing slop`, `clean text spacing` | CJK Spacing Normalizer. Cleans extraneous spaces, decomposes Jamo to NFC, and normalizes inter-script borders |

### ![Digital Forensics & Data Analysis](https://img.shields.io/badge/Forensics_%26_Analysis-0F172A?style=flat-square) Digital Forensics & Data Analysis (4 skills)

| Skill | Trigger | Description |
| :--- | :--- | :--- |
| **exif-extractor** | `exif`, `metadata`, `hashes`, `spoofing`, `signature check` | Extract metadata, calculate hashes (MD5, SHA-256), and verify file magic signatures to detect extension spoofing |
| **sqlite-analyzer** | `sqlite`, `db`, `query database`, `database schema` | Inspect SQLite database schemas, count records, and search tables for keyword patterns |
| **macb-timeline** | `timeline`, `MACB`, `directory timeline`, `file events` | Generate chronological MACB (Modified, Accessed, Changed, Created) timeline from files in a directory |
| **duplicate-finder** | `duplicate finder`, `find duplicates`, `check duplicates`, `md5 match` | Scan a directory recursively to identify duplicate files using MD5 hashes |
...
## 🔍 Digital Forensics & Data Analysis Tools

lazyantigravity provides a specialized suite of tools designed for security researchers, digital forensic investigators, and data analysts. These scripts automate deep content verification, file metadata scanning, database exploration, and duplicate detection:

1. **`duplicate-finder` (MD5 Duplicate File Finder)**:
   - **Command**: `node scripts/find-duplicates.mjs <directory_path>`
   - **Optimization**: Uses a high-performance two-pass algorithm. It first groups all files by size and only computes MD5 hashes for files of matching sizes, drastically reducing disk read operations.
   - **Output**: Clearly groups duplicate files by their MD5 hash and size, listing absolute paths for direct verification.

2. **`exif-extractor` (Exif Metadata & Signature Spoofing Detector)**:
   - **Command**: `node scripts/extract-metadata.mjs <file_path>`
   - **Description**: Extracts Exif metadata, calculates MD5 & SHA-256 hashes, and checks for extension spoofing by matching magic bytes signatures (e.g. JPEG, PNG, ZIP, PDF, SQLite, EXE) against the file extension.

3. **`sqlite-analyzer` (SQLite Schema & Keyword Searcher)**:
   - **Command**: `node scripts/analyze-sqlite.mjs <db_path>`
   - **Description**: Automatically inspects SQLite database schemas, counts records for all tables, and performs deep keyword pattern searches across all text fields.

4. **`macb-timeline` (Chronological Event Timeline Generator)**:
   - **Command**: `node scripts/generate-timeline.mjs <directory_path>`
   - **Description**: Scans a directory recursively to extract MACB (Modified, Accessed, Changed, Created) timestamps and compiles a chronological timeline of filesystem activities.
...
3. **CJK Localization & TUI Alignment Auditing**
   ...
   - **Improvement**: Extended the Pass A/B check guidelines in [visual-qa/SKILL.md](file://./packages/omo-codex/plugin/skills/visual-qa/SKILL.md) to explicitly require auditing for CJK wrap styles (`keep-all`, `break-all`), glyph drop (tofu), and TUI box-drawing grids.

4. **Parallelized LSP Diagnostics**
   - **Gemini Trait**: Fast, type-safe generation requires immediate feedback on compile errors.
   - **Improvement**: Parallelized the LSP diagnostics retrieval loop inside [prompt-amplifier.mjs](file://../scripts/prompt-amplifier.mjs) using `Promise.all`, reducing the maximum submit hook latency from 4.5 seconds down to a max of 1.5 seconds.
...
- **29 Specialized Skills**: From ultraresearch swarms to visual QA to TDD workflows.
...
## 🛡️ Recent Stability & Privacy Enhancements

1. **Sibling Dependency Materialization**:
   - Resolved packaging defects where external local sibling folder dependencies (such as `shared-skills` and `shared-lsp`) were omitted from cache build targets. The cache installer now automatically copies (materializes) sibling folders into the plugin directory and rewrites `package.json` dependencies to point locally (`file:./shared-lsp`).
2. **Enhanced Privacy (PostHog GeoIP Disabled)**:
   - Configured `disableGeoip: true` in the telemetry client configuration, disabling IP-based location mapping. Only a hashed hostname identifier (`sha256("omo-codex:" + hostname)`) is collected for basic daily active metrics. **No source code, file contents, or sensitive keys are ever sent externally.**
3. **Escalated Doctor Severity**:
   - Upgraded version mismatches between loaded plugins and cached builds from a `warning` to a hard `error` to prevent agent runtime misbehavior.
...
To protect user privacy, **PostHog GeoIP lookup is disabled by default** (`disableGeoip: true`) to prevent device geolocation tracking. Once per day at session start, only a hashed identifier (`sha256("omo-codex:" + hostname)`) is transmitted for anonymous usage indicators. **No source code, file contents, or sensitive credentials are ever sent externally.**
