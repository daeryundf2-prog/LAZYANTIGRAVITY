...
For every background review lane, require progress signals before long passes:
`WORKING: <lane> - <current phase>` while the lane is still making progress,
and `BLOCKED: <reason>` only when progress stops. A mailbox timeout only means no new mailbox update arrived; it is not a failure by itself.
Fallback only when the lane returns without the deliverable, sends only an ack
after follow-up, reports `BLOCKED:`, or is confirmed no longer running.
