...
- **High Density (80–100)**:
  > "Updated `model-catalog.json` `antigravity.current.model` to `gemini-3.8-flash-high` with `canTierRoute: true`, `hostEnforced: false`. Documented `invoke_subagent` `Subagents[].Model` as an agent hint. Verified 124 unit tests pass."
