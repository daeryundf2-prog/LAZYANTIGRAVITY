---
name: archify
description: Create polished, validated architecture, workflow, sequence, data-flow, and lifecycle/state diagrams as explorable standalone HTML with inline SVG, dark/light themes, optional trace motion, and PNG/JPEG/WebP/SVG/WebM export. Accept plain-language requirements or pasted Mermaid flowchart, sequenceDiagram, and stateDiagram input; inspect repository evidence when the diagram must reflect real code. Use when the user asks to visualize system architecture, infrastructure, cloud/security/network topology, technical workflows, API call sequences, request lifecycles, data pipelines, ETL/ELT, data lineage, state machines, or to convert/beautify Mermaid. 한국어 트리거: 아키텍처 다이어그램, 시스템 구조 시각화, 워크플로/시퀀스/데이터플로/상태 다이어그램, 인프라 토폴로지, API 호출 흐름, 데이터 파이프라인 도식화, Mermaid 변환·미화.
license: MIT
metadata:
  version: "2.16"
  author: tt-a1i
  based_on: Cocoon-AI/architecture-diagram-generator (MIT, v1.0)
  vendored: "LazyAntigravity shared-skills (2026-08-30) — trimmed; remote update checker removed"
---
...
> **Vendored copy note (LazyAntigravity):** this build removes the upstream
> `scripts/check-update.mjs` remote-update checker and its "Update awareness"
> workflow. Sessions must not phone home; upgrades are handled by the
> LazyAntigravity repository, not by the skill at runtime.
