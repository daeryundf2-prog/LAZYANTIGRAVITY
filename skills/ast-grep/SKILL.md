...
## Runtime note (Antigravity)

The upstream version of this skill references `mcp__ast_grep_*` MCP tools bundled with OmO native. Those tools are **NOT registered in Antigravity** — do not attempt to call them. In this environment the path is the `sg` CLI (or `ast-grep`) resolved via `command -v sg`; when absent, run `install.sh`/`install.ps1` or the helper's `install` command, and if installation is not possible, fall back to `rg` and say the AST-shape search was not performed.
