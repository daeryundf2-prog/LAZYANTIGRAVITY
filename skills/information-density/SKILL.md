...
- **High Density (80–100)**:
  > "Updated `model-catalog.json` `antigravity.current.model` to `gemini-3.7-flash-high` with `canTierRoute: true` and `tierMap.verifier=pro`. Documented `invoke_subagent` model_tier routing. Verified 119 unit tests pass."
