...
8. RECORD exactly one result:
   - PASS: `omo ulw-loop record-evidence --goal-id <id> --criterion-id <id> --status pass --evidence "file:///absolute/path/to/evidence.txt | <cleanup receipt>" --json` (Note: evidence MUST be real output via `>`/`tee`; never hand-write success logs.)
