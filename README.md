...
7. **Dynamic Target LSP Extensions & Sorting (LSP 대상 파일 동적 파싱 및 우선순위화)**
   - **특성**: 검사 대상 확장자가 `ts, go, py` 등으로 고정되어 타 언어 환경 대응이 불가능했으며, 무작위 순서로 파일을 진단하여 신규 빈 파일이 먼저 스캔되는 우선순위 결함이 존재했습니다.
   - **개선점**: 기본 확장자 풀을 22개 범용 언어군으로 대폭 늘렸고, 프로젝트 내 `.codex/lsp-client.json` 설정이 감지될 경우 해당 파일에 구성된 언어를 파싱하여 검사 대상 확장자로 동적 추가하게 구현했습니다. 또한, git status 상에서 수정된 파일(`M`)에 가중치를 부여해 우선적으로 LSP 진단되게 정렬 로직을 추가했습니다.
