...
3. **Existing project with `DESIGN.md` or a component system:** read it, follow it, and update it before implementation only when the requested work needs a new token, primitive, state, motion rule, accessibility constraint, accepted debt, or reference-fidelity requirement.
   ...
   - **Astryx Design System (`xds` MCP server)**: When working on an Astryx project, connect to the `xds` MCP server to dynamically query component patterns (`xds/search`) and retrieve props or usage specifications (`xds/get`). Use dense output parameters where available to minimize token overhead.
