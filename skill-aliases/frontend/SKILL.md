...
3. **Existing project with `DESIGN.md` or a component system:** read it, follow it, and update it before implementation only when the requested work needs a new token, primitive, state, motion rule, accessibility constraint, accepted debt, or reference-fidelity requirement.
   - **Token-Scoped Dynamic Loading**: Loading the entire `DESIGN.md` in every prompt causes massive token bloat (up to 92% overhead) and context drift. Instead, dynamically load only the specific token groups (e.g., colors, spacing, typography) required for the target component.
   - **Component Reuse Priority**: Do not generate new custom styled markup or duplicate layout patterns if an existing primitive component (from the workspace design system or libraries like Shadcn/Tailwind) can be reused. Enforce existing abstractions first.
   - **Astryx Design System (`xds` MCP server)**: When working on an Astryx project, connect to the `xds` MCP server to dynamically query component patterns (`xds/search`) and retrieve props or usage specifications (`xds/get`). Pass the optional `section` parameter to `xds/get` to retrieve targeted documentation segments and avoid context bloat. Always write clean JSDoc comments for any newly added or swizzled components to keep them machine-readable.
