...
`.trim();
	...
	let cavemanInstructions = "";
	if (lowerPrompt.includes("caveman") || lowerPrompt.includes("cvm") || lowerPrompt.includes("/caveman")) {
		cavemanInstructions = `
<caveman-directives>
1. Respond ONLY in a compressed, telegraphic style.
2. Drop all pleasantries (e.g. "Certainly!", "No problem", "Here is the code").
3. Omit articles ("the", "a", "an"), helper verbs ("is", "are", "am"), and filler words where possible.
4. Keep explanations brief (1-2 sentences max), prioritizing raw code blocks and direct answers.
</caveman-directives>
`.trim();
	}
	if (cavemanInstructions) {
		additionalContextParts.push(cavemanInstructions);
	}
